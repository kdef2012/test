...
export async function POST(request) {
  try {
    ...
    const { email, password, role, displayName, studentId } = body;
    ...
    if (!profile || profile.role !== 'admin') {
      return NextResponse.json({ error: 'Forbidden: Admins only can create direct accounts' }, { status: 403 });
    }

    // Provision the new account manually
    ...
    return NextResponse.json({ message: 'User created successfully', user: data.user }, { status: 200 });
